...
describe("NAAI ERP JSON-first CLI client", () => {
  it("runs evidence-based expense funding inference", async () => {
    const fetchFn = vi
      ...
      .mockImplementation(() =>
        Promise.resolve(
          new Response(JSON.stringify({ data: { planHash: "plan-1" } }), { status: 200 }),
        ),
      );
    ...
    expect(fetchFn).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        method: "POST",
        body: JSON.stringify({ reason: "audit live funding" }),
      }),
    );
    ...
    expect(fetchFn.mock.calls[1]?.[1]).toEqual(
      expect.objectContaining({
        headers: expect.objectContaining({ "idempotency-key": "funding-commit-1" }),
      }),
    );
  });
  ...
  it("keeps inbound events read-only after manual replay removal", async () => {
    const fetchFn = vi.fn();
    const client = new NaaiErpClient(
      { baseUrl: "http://api", organizationId: "org-a", token: "secret" },
      fetchFn,
    ...
    expect(() =>
      client.request("inbound-events", "replay", {}, "msg-1", undefined, "replay-key"),
    ).toThrow("read-only");
