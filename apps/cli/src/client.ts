      ...
      isForecastComponent && action === "delete"
        ...
        : action === "list" ||
          ...
          : action === "update"
            ? "PATCH"
            : "POST";
    const path = isReportSnapshot
      ? action === "reproduce"
        ? (() => {
            if (!key || !expectedVersion || !/^[1-9]\d*$/.test(expectedVersion)) {
              throw new Error("report-snapshots reproduce requires --key and a positive --version");
            }
