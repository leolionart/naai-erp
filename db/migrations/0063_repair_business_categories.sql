-- Repair migration for environments where the category feature was deployed while
-- migration 0062 was already marked as applied but its table was not present.
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'business_category_type') THEN
...
CREATE INDEX IF NOT EXISTS business_categories_active_name_idx
