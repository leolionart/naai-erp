...
- **Watchtower tag (2026-08-09):** Every image publication updates `latest` in addition to `main`
  and the immutable SHA tag. The production Compose default uses `latest` for API, web, worker and
  one-shot migration images so Watchtower can detect a new digest.
