...
- Atomic publish depends on the PostgreSQL transaction covering validation, composition calculation, state transition and snapshot persistence. Exact-commit CI must execute the integration test; static code presence alone is not final durability proof.
...
- Local Playwright is green at 37/37. Exact-commit PostgreSQL and complete GitHub quality-job proof are still required before completion.
